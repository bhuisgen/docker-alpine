...
template {
  ...
  command     = "/bin/chown root:dovecot /etc/dovecot/dovecot-sql.conf.ext && /bin/sh -c 'test -d /run/service/dovecot && doveadm reload || true'"
  perms       = 0640
}
...
template {
  ...
  command     = "/bin/chown root:dovecot /etc/dovecot/conf.d/auth-sql.conf.ext && /bin/sh -c 'test -d /run/service/dovecot && doveadm reload || true'"
  perms       = 0644
}
